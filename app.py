...
if not df.empty:
    ...
    if not valid_df.empty:
        ...
        # 3. Округляем проценты до 1 знака
        display_df['%R'] = display_df['%R'].round(1)
        display_df['%S'] = display_df['%S'].round(1)
        display_df['%I'] = display_df['%I'].round(1)
        
        # 🔥 4. КРИТИЧЕСКИ ВАЖНО: Явно выбираем ПРАВИЛЬНЫЙ порядок колонок ПЕРЕД переименованием
        # Теперь порядок: Антибиотик, Total, S, %S, I, %I, R, %R
        ...
        # 5. Теперь переименование совпадает с реальным порядком колонок
        display_df.columns = [
